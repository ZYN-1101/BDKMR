...
class DBCFastRCNNOutputs(FastRCNNOutputs):
    """
    A class that stores information about outputs of a Fast R-CNN head.
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            pred_class_logits=pred_class_logits_regu,
            pred_proposal_deltas=pred_proposal_deltas,
            proposals=proposals,
            smooth_l1_beta=smooth_l1_beta
        )
        self.pred_class_logits_normal = pred_class_logits_normal

    def normal_softmax_cross_entropy_loss(self):
        """
        Compute the softmax cross entropy loss for box classification.
        ...
        """
        return F.cross_entropy(
            self.pred_class_logits_normal, self.gt_classes, reduction="mean"
        )
    ...
    def losses(self):
        ...
        return {
            "loss_cls": self.softmax_cross_entropy_loss(),
            "loss_cls_normal": self.normal_softmax_cross_entropy_loss(),
            "loss_box_reg": self.smooth_l1_loss(),
        }
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class DBCFastRCNNOutputLayers(FastRCNNOutputLayers):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(DBCFastRCNNOutputLayers, self).__init__(
            cfg=cfg,
        ...
        self.cls_score_regu = nn.Linear(input_size, num_classes + 1, bias=False)
        nn.init.normal_(self.cls_score_regu.weight, std=0.01)

        self.cls_score_normal = nn.Linear(input_size, num_classes + 1, bias=False)
        nn.init.normal_(self.cls_score_normal.weight, std=0.01)
    ...
    def forward(self, x):
        ...
        # normalize weight
        self.cls_score_regu.weight.data = F.normalize(self.cls_score_regu.weight.data, p=2, dim=1)
        self.cls_score_normal.weight.data = F.normalize(self.cls_score_normal.weight.data, p=2, dim=1)

        scores_regu = self.cls_score_regu(x.detach())
        scores_normal = self.cls_score_normal(x)

        return scores_regu, scores_normal, proposal_deltas