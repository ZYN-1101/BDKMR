...
from bdkmr.modeling.roi_heads import build_roi_heads
...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs, iter=0):
        if not self.training:
            return self.inference(batched_inputs)
        assert "instances" in batched_inputs[0]
        gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        proposal_losses, detector_losses, _, _ = self._forward_once_(batched_inputs, gt_instances, iter)
        losses = {}
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None, iter=0):
        ...
        results, detector_losses = self.roi_heads(images, features_de_rcnn, proposals, gt_instances, iter)
